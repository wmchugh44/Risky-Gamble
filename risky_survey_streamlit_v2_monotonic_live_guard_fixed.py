...
def check_monotonic_violation(choices, amounts, new_choice, idx):
    ...
    Assumes `amounts` is in descending order (largest first).
    """
    temp = list(choices)
    temp[idx] = new_choice
    for i, ch in enumerate(temp):
        if ch is None:
            continue
        if ch == "prospect":
            # If you rejected a sure amount at i, you must reject any smaller amount j > i
            for j in range(i+1, len(temp)):
                if temp[j] == "sure":
                    return True, (
                        f"This choice violates monotonicity — if you prefer the gamble over ${amounts[i]}, "
                        f"you must also prefer it over any smaller sure amount like ${amounts[j]}."
                        "\n\nMonotonicity (Tversky & Kahneman, 1992): "
                        "If a gamble is better than some amount of money, it should be better than any smaller amount."
                    )
        if ch == "sure":
            # If you accepted a sure amount at i, you must accept any larger amount j < i
            for j in range(0, i):
                if temp[j] == "prospect":
                    return True, (
                        f"This choice violates monotonicity — if you prefer ${amounts[i]} for sure, "
                        f"you must also prefer any larger sure amount like ${amounts[j]}."
                        "\n\nMonotonicity (Tversky & Kahneman, 1992): "

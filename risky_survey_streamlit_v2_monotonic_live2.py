...
def describe_prospect(prospect):
    ...
    if len(outs) == 2 and len(probs) == 2:
        o1, o2 = outs
        p1, p2 = probs  # p1 corresponds to o1, p2 to o2
        def fmt_outcome(x):
            if x == 0:
                return "nothing"
            sign = "win" if x > 0 else "lose"
            return f"{sign} ${abs(x):.0f}".replace(".00", "")
...
if not st.session_state.started:
    ...
    if current >= total_problems:
        ...
        # Live monotonic enforcement (no form): selecting a radio triggers a rerun and auto-fills other rows
        # without disabling them. Users can still change any row; we recompute a monotone pattern each time.
        def _set_choice(key, idx):
            val = st.session_state.get(key)
            if val is None:
                return
            st.session_state.current_choices[idx] = "prospect" if val == "Prefer Gamble" else "sure"
            # Recompute monotone pattern immediately
            _choices = st.session_state.current_choices
            first_sure_idx = None
            last_prospect_idx = None
            for _i, ch in enumerate(_choices):
                if ch == "sure" and first_sure_idx is None:
                    first_sure_idx = _i
                if ch == "prospect":
                    last_prospect_idx = _i if last_prospect_idx is None else max(last_prospect_idx, _i)
            for j in range(len(_choices)):
                if first_sure_idx is not None and j < first_sure_idx:
                    _choices[j] = "sure"
                if last_prospect_idx is not None and j > last_prospect_idx:
                    _choices[j] = "prospect"

        # Determine current choices (already possibly updated by callbacks)
        choices = st.session_state.current_choices

        # Render radios; preselect from current choices; do NOT disable
        ...
        if st.button("Continue", use_container_width=True):
            ...
            # Enforce monotone pattern one last time before validating
            first_sure_idx = None
            last_prospect_idx = None
            for _i, ch in enumerate(choices):
                if ch == "sure" and first_sure_idx is None:

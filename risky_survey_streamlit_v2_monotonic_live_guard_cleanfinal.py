    ...
    for i, ch in enumerate(temp):
        ...
        if ch == "prospect":
            for j in range(i+1, len(temp)):
                if temp[j] == "sure":
                    msg = (
                        f"**This choice violates monotonicity.**\n\n"
                        f"If you prefer the **gamble** over ${amounts[i]:,.2f}, you must also prefer it over any **smaller** sure amount like ${amounts[j]:,.2f}.\n\n"
                        f"*Monotonicity (Tversky & Kahneman, 1992): If a gamble is better than some amount of money, it should be better than any smaller amount.*"
                    )
                    return True, msg
        if ch == "sure":
            for j in range(0, i):
                if temp[j] == "prospect":
                    msg = (
                        f"**This choice violates monotonicity.**\n\n"
                        f"If you prefer a **sure amount** of ${amounts[i]:,.2f}, you must also prefer any **larger** sure amount like ${amounts[j]:,.2f}.\n\n"
                        f"*Monotonicity (Tversky & Kahneman, 1992): If a certain amount is better than the gamble, any larger amount should also be better.*"
                    )
                    return True, msg
    return False, ""
...
if not st.session_state.started:
    ...
    if current >= total_problems:
        ...
        # Live radios with INSTANT monotonic guard: if a selection violates the rule, we revert it and show an error.
        ...
        # Render radios; keep all enabled; default from session state
        choices = st.session_state.current_choices
